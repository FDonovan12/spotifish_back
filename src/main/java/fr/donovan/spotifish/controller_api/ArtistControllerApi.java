...
public class ArtistControllerApi {
    ...
    @GetMapping(path = UrlRoute.URL_ARTIST + "/{id}")
    @JsonView(JsonViews.ArtistShowJsonViews.class)
    public CustomResponse<Artist> show(@PathVariable String id) {
        return new CustomResponse<>(HttpStatus.OK.value(), "ArtistControllerApi - show("+id+")", "Artist", this.artistService.getObjectById(id));
    }

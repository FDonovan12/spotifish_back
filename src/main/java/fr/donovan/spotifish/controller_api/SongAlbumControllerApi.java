...
public class SongAlbumControllerApi {
    ...
    public CustomResponse<List<SongAlbum>> list() {
        return new CustomResponse<>(HttpStatus.OK.value(), "SongAlbumControllerApi - list()", "SongAlbum", this.songAlbumService.findAll());
    }
    ...
    public CustomResponse<SongAlbum> show(@PathVariable String slug) {
        return new CustomResponse<>(HttpStatus.OK.value(), "SongAlbumControllerApi - show("+slug+")", "SongAlbum", this.songAlbumService.getObjectBySlug(slug));
    }
    ...
    public CustomResponse<SongAlbum> create(@Valid @RequestBody SongAlbumDTO songAlbumDTO) {
        return new CustomResponse<>(HttpStatus.CREATED.value(), "SongAlbumControllerApi - create()", "SongAlbum", songAlbumService.persist(songAlbumDTO));
    }
    ...
    public CustomResponse<SongAlbum> update(@Valid @RequestBody SongAlbumDTO songAlbumDTO, @PathVariable SongAlbumId id) {
        return new CustomResponse<>(HttpStatus.OK.value(), "SongAlbumControllerApi - update("+id+")", "SongAlbum", songAlbumService.persist(songAlbumDTO, id));
    }
    
    @DeleteMapping(path = UrlRoute.URL_SONGALBUM_DELETE + "/{id}")
    public CustomResponse<Boolean> delete(@PathVariable SongAlbumId id) {
        return new CustomResponse<>(HttpStatus.OK.value(), "SongAlbumControllerApi - delete("+id+")", "SongAlbum", songAlbumService.delete(id));
    }
}